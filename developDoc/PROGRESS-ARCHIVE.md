...
> `PROGRESS.md` 只保留**当前**进度；本文件保存已翻页的历史条目（原文照搬，不再更新）。
> 归档范围：2026-08-23 v0.3.0 ~ 2026-09-18 v0.4.0（含各次踩坑记录）。
> 维护方式：`PROGRESS.md` 顶部持续追加新条目，阶段性把旧条目整体搬到本文件末尾。
> **注意**：其中仍会咬人的坑已抽成一行一条收进 `AGENTS.md`「已知陷阱」，
> 本文件对应条目只作来龙去脉参考。
