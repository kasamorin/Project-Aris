...
- **无密码模式（2026-09-18 定案，取代原「未配置即拒绝登录」）**：
  - 未配置 `ARIS_WEBUI_PASSWORD` 时进入**免鉴权模式**：中间件走显式分支整段放行
    （不是"任意匹配即通过"，避免松匹配式绕过）；`/login` 直接 302 回 `/`，
    导航隐藏「退出」并显示「免鉴权模式（本机）」。
  - **护栏**：免鉴权模式下监听地址被强制降级为回环（`resolve_bind_host()`）——
    即便传 `--host 0.0.0.0` 也只会绑 `127.0.0.1`，并在日志里明确提示原因与出路。
    这样"本机开发不想配密码"与"不能裸奔到局域网"两件事同时成立。
  - 启动日志另打一条「WebUI 运行在免鉴权模式」的 warning，绝不静默放开。
- 配置了密码时行为与以前完全一致（重定向登录 + 7 天会话 + 限流）。
